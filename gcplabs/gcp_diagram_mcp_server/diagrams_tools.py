...
import signal
import uuid
from gcplabs.gcp_diagram_mcp_server.models import (
    DiagramExampleResponse,
    DiagramGenerateResponse,
    DiagramIconsResponse,
    DiagramType,
)
from gcplabs.gcp_diagram_mcp_server.scanner import scan_python_code
...
async def generate_diagram(
    ...
    if os.path.isabs(filename):
        ...
        # (for relative paths with directories like "path/to/diagram")
...
def get_diagram_examples(diagram_type: DiagramType = DiagramType.ALL) -> DiagramExampleResponse:
    ...
    if diagram_type in [DiagramType.GCP, DiagramType.ALL]:
        examples['gcp_basic'] = """from diagrams.gcp.compute import GCE
from diagrams.gcp.database import SQL
from diagrams.gcp.network import LoadBalancing

with Diagram("Web Service", show=False):
    LoadBalancing("lb") >> GCE("web") >> SQL("userdb")
"""
...
"""
    ...
    if diagram_type in [DiagramType.GCP, DiagramType.ALL]:
        examples[
            'gcp_grouped_instances'
        ] = """from diagrams.gcp.compute import GCE
from diagrams.gcp.database import SQL
from diagrams.gcp.network import LoadBalancing

with Diagram("Grouped Instances", show=False, direction="TB"):
    LoadBalancing("lb") >> [GCE("worker1"),
                          GCE("worker2"),
                          GCE("worker3"),
                          GCE("worker4"),
                          GCE("worker5")] >> SQL("events")
"""
...
"""
        ...
        examples['gcp_data_flow'] = """from diagrams.gcp.analytics import BigQuery, Dataflow
from diagrams.gcp.compute import GKE
from diagrams.gcp.pubsub import Pubsub
...
with Diagram("Data Flow", show=False):
    ...
    with Cluster("Event Ingestion"):
        ...
        with Cluster("Processing"):
            handlers = [Dataflow("proc1"),
                        Dataflow("proc2"),
                        Dataflow("proc3")]

    store = GCS("events store")
...
with Diagram("Vertex AI Prediction", show=False, direction="LR"):
    user = User("User")
    
    with Cluster("Google Cloud"):
        gcs = GCS("Input Data")
        vertex_ai = AIPlatform("Vertex AI Prediction")
        bq = BigQuery("Logging")
