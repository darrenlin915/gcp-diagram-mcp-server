...
"""gcp-diagram-mcp-server implementation.
...
from gcplabs.gcp_diagram_mcp_server.diagrams_tools import (
    generate_diagram,
    get_diagram_examples,
    list_diagram_icons,
)
from gcplabs.gcp_diagram_mcp_server.models import DiagramType
...
2. get_diagram_examples:
   - Request example code for the diagram type you need (gcp, sequence, flow, class, k8s, onprem, custom, or all)
...
- GCP architecture diagrams: Cloud infrastructure and services
...
async def mcp_get_diagram_examples(
    diagram_type: DiagramType = Field(
        ...
        description='Type of diagram example to return. Options: gcp, sequence, flow, class, k8s, onprem, custom, all',
    ),
):
    ...
    EXAMPLE CATEGORIES:
    - gcp: GCP cloud architecture diagrams
...
async def mcp_list_diagram_icons(
    provider_filter: Optional[str] = Field(
        default=None, description='Filter icons by provider name (e.g., "gcp", "k8s")'
    ),
